#!/bin/bash
# 献立まるっとプランナー iOS v1.7(build21) を、1画面1ステップ込みでビルド→App Store Connectへアップロードする（2026-09-07）
# 打つのはこれ1本:  bash ~/git/kondate-planner/native/release_v1.7.sh
...
echo "⑥ 次: python3 ~/git/MIKA_VAULT/06_Projects/20260721_献立アプリUIUX監査/tools/asc.py builds  でVALIDを確認 → create_version 1.7 → set_build → submit（AIがやる）"
