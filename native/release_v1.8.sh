#!/bin/bash
# 献立まるっとプランナー iOS v1.8(build22) を、AdMobバナー広告（本番ID）込みでビルド→App Store Connectへアップロードする（2026-09-20）
# 打つのはこれ1本:  bash ~/git/kondate-planner/native/release_v1.8.sh
...
echo "⑥ 次: python3 ~/git/MIKA_VAULT/06_Projects/20260721_献立アプリUIUX監査/tools/asc.py builds  でVALIDを確認 → create_version 1.8 → set_build → submit（AIがやる）"
